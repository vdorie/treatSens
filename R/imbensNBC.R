...
llik <- function(b, Y, W, X, alpha, delta) {
	...
	loglik = log(normal.lik(beta,tau,sigma2,Y,W,X,0)*logistic.lik(gamma, W, X, 0) +
			normal.lik(beta,tau,sigma2,Y,W,X,delta)*logistic.lik(gamma, W, X, alpha) )
	return(sum(loglik))
}
